...
const CATEGORIES = [
  {
    id: "frases",
    eu: "BETIKO GELDITUKO DIREN ESALDIRIK ONENAK",
    es: "Mejores frases que se quedan para siempre",
    icon: "micro",
    quote: "Hitz batzuk ez dira esaten. Grabatu egiten dira.",
  },
  {
    id: "osasuna",
    eu: "OSASUNERAKO IBILBIDE ONENA",
    es: "Mejor trayectoria hacia la salud",
    icon: "health",
    quote: "Alfonbra gorria, asmo onak eta tentazio txarrak.",
  },
  {
    id: "laguna",
    eu: "LAGUNEN LAGUNIK ONENA",
    es: "Mejor amigo de sus amigos",
    icon: "crown",
    quote: "Leialtasuna, baina estiloarekin.",
  },
  {
    id: "birjaiotze",
    eu: "BIRJAIOTZE ONENA",
    es: "Mejor renacer",
    icon: "flower",
    quote: "Errautsen artetik, trajea pixka bat zimurtuta.",
  },
  {
    id: "kaosa",
    eu: "KAOSAREN ETA ORDENAREN ARTEKO OREKARIK ONENA",
    es: "Mejor equilibrio entre caos y orden",
    icon: "scale",
    quote: "Desastre bat, baina harrigarriro ondo antolatua.",
  },
  {
    id: "altxatu",
    eu: "ERORTZEN ETA BERRIZ ALTXATZEN ONENA",
    es: "Mejor en caer y volver a levantarse",
    icon: "rise",
    quote: "Erori, begiratu ingurura, eta berriz zutik.",
  },
];

function BottleTrophy({ small = false }) {
  return (
    <div className={`relative mx-auto ${small ? "h-20 w-16" : "h-40 w-28"}`}>
...
function CategoryIcon({ type }) {
  const base = "h-24 w-24 rounded-full border border-amber-400/40 bg-black/35 flex items-center justify-center shadow-2xl mx-auto";
  if (type === "micro") return <div className={base}><span className="text-6xl">“</span><Beer className="h-8 w-8 text-amber-300"/><span className="text-6xl">”</span></div>;
  if (type === "health") return <div className={base}><span className="text-5xl">👟</span><Beer className="h-7 w-7 text-amber-300 ml-1"/></div>;
  if (type === "crown") return <div className={base}><Crown className="h-14 w-14 text-amber-300"/></div>;
  if (type === "flower") return <div className={base}><span className="text-6xl">✺</span></div>;
  if (type === "scale") return <div className={base}><span className="text-6xl">⚖</span></div>;
  return <div className={base}><span className="text-6xl">↗</span></div>;
}
  ...
  const saveVotes = async () => {
    ...
    const rows = CATEGORIES.map((cat) => {
      ...
      return {
        voter,
        category_id: cat.id,
        first_choice: selected[0] || null,
        second_choice: selected[1] || null,
        third_choice: selected[2] || null,
      };
    });
    ...
    <div className="min-h-screen bg-[#120908] text-[#f4d18a] overflow-hidden font-sans">
      ...
      <main className="relative z-10 mx-auto flex min-h-screen max-w-md flex-col px-4 py-6">
        <AnimatePresence mode="wait">
          {screen === "home" && (
            <motion.section key="home" initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -18 }} className="flex min-h-[92vh] flex-col justify-between rounded-[2rem] border border-amber-300/25 bg-black/35 p-5 shadow-2xl backdrop-blur-sm">
            ...
            <motion.section key={currentCategory.id} initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} className="rounded-[2rem] border border-amber-300/25 bg-black/40 p-4 shadow-2xl">
              ...
              <div className="mt-4 rounded-2xl border border-amber-300/20 bg-black/35 p-3">
                <div className="mb-3 flex items-center justify-between">
                  <span className="font-black uppercase text-red-300">Aukeratu 3 / Elige 3</span>
                  <span className="rounded-full bg-amber-400 px-3 py-1 text-sm font-black text-black">{selected.length}/3</span>
                </div>
                <div className="relative mb-3">
                  <Search className="absolute left-3 top-3 h-4 w-4 text-amber-100/50"/>
                  <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Bilatu izena / Busca un nombre..." className="w-full rounded-xl border border-amber-300/20 bg-black/50 py-3 pl-10 pr-3 text-amber-100 placeholder:text-amber-100/40 outline-none" />
                </div>
                <div className="max-h-[42vh] space-y-2 overflow-y-auto pr-1">
                  {filteredFriends.map((name) => {
                    ...
                    const active = rank !== -1;
                    return (
                      <button key={name} onClick={() => selectName(name)} className={`flex w-full items-center justify-between rounded-xl border px-3 py-3 text-left font-bold transition ${active ? "border-amber-300 bg-amber-400 text-black" : selected.length >= 3 ? "border-amber-300/10 bg-black/20 text-amber-100/40" : "border-amber-300/20 bg-black/35"}`}>
                        <span>{name}</span>
                        {active && <span className="text-lg">{rank === 0 ? "🥇" : rank === 1 ? "🥈" : "🥉"}</span>}
                      </button>
                    );
                  })}
                </div>
              ...
              <div className="mt-4 flex gap-2">
                ...
                <button onClick={nextCategory} disabled={selected.length === 0 || isSaving} className="flex flex-1 items-center justify-center gap-2 rounded-2xl bg-red-800 px-5 py-3 font-black uppercase tracking-widest disabled:opacity-40">
                  {isSaving ? "Gordetzen..." : categoryIndex === CATEGORIES.length - 1 ? "Bidali botoak" : "Hurrengoa / Siguiente"} <ArrowRight className="h-5 w-5"/>
                </button>
              </div>
            </motion.section>
          )}
